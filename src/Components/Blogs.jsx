  ...
  return (
    <div className="blogs">
      ...
      <div className="blogs-right">
        ...
        {submitted && <p className="submission-message">İleti Gönderildi!</p>}
        <div className={`blogs-right-form ${showForm ? 'visible' : 'hidden'}`}>
          <h1>{isEditing ? 'Edit Post' : 'YENİ İLETİ'}</h1>
          <form onSubmit={handleSubmit}>
            <div className="img-upload">
              <label htmlFor="file-upload" className="file-upload">
                <i className="bx bx-upload"></i> Resim Yükle
              </label>
              <input type="file" id="file-upload" onChange={handleImageChange} />
            </div>
            <input
              type="text"
              placeholder="Başlık ekle... (Max 60 Karakter)"
            ...
            <button type="submit" className="submit-btn">
              {isEditing ? 'Update Post' : 'Gönder'}
            </button>
          </form>
        </div>
        <button className="blogs-close-btn" onClick={onBack}>
          Geri <i className="bx bx-chevron-right"></i>
        </button>
      </div>
    </div>
  )
